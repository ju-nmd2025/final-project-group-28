...
function setup() {
    ...
    p3.visited = false;//it only exists if it has not been visited
...
function draw() { 
    ...
    for (let i = platforms.length -1; i >= 0; i--) {//Index never is above 0 so it never stops //--i means the loop is backwards
        const p = platforms [i];//p is current platform with i a number from array//array indexing= [i]
        
        //skip broken platforms
        if(p.removed) {//checks if a removed platform exists
        ...
        if (p.type === TYPE_MOVING) {//they're the same
    ...
    for (let plat of platforms) plat.draw();//makes platforms plat.draw 
     ...
     for (let plat of platforms) {//loops through all platforms exept breaking plats
